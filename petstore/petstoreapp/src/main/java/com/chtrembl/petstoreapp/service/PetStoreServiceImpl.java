...
public class PetStoreServiceImpl implements PetStoreService {
	...
	// private WebClient orderItemReseverFuncApp = null;
	...
	public void initialize() {
		...
		this.orderServiceWebClient = WebClient.builder().baseUrl(this.containerEnvironment.getPetStoreOrderServiceURL())
				.build();
		// this.orderItemReseverFuncApp =
		// WebClient.builder().baseUrl(this.containerEnvironment.getOrderedItemFuncAppURL())
		// .build();
	}
	...
	public Collection<Pet> getPets(String category) {
		...
		this.sessionUser.getTelemetryClient().trackEvent(String
				.format("PetStoreApp user is requesting under the session ID: %s", this.sessionUser.getSessionId()),
				this.sessionUser.getCustomEventProperties(), null);
		try {
			...
			this.sessionUser.getTelemetryClient()
					.trackEvent(
							String.format("PetStoreApp user is requesting under the session ID: %s",
	...
	public Collection<Product> getProducts(String category, List<Tag> tags) throws Exception {
		...
		this.sessionUser.getTelemetryClient()
				.trackEvent(
						String.format("PetStoreApp user %s is requesting under the session ID: %s",
								this.sessionUser.getName(), this.sessionUser.getSessionId()),
						this.sessionUser.getCustomEventProperties(), null);

		try {
			// products.get(2);// Access randomly from an empty list will break the code
			Consumer<HttpHeaders> consumer = it -> it.addAll(this.webRequest.getHeaders());
		...
		WebClientException wce) {
			...
			throw new Exception("Cannot move further with getting product");
		}
		return products;
	}
	...
	public void updateOrder(long productId, int quantity, boolean completeOrder) {
		...
		try {
			...
			if (updatedOrder.isComplete()) {
				Order finalOrder = this.retrieveOrder(this.sessionUser.getSessionId());
				String connectionString = System.getenv("AzureWebJobsServiceBus");
				String srvcBusUrl = System.getenv("AzureServiceBusUrl");
				String srvcBusQueue = System.getenv("AzureServiceBusQueue");
				logger.info("connectionString: {}, srvcBusUrl: {}, srvcBusQueue:{}", connectionString, srvcBusUrl,
						srvcBusQueue);
				ServiceBusSenderClient senderClient = new ServiceBusClientBuilder().fullyQualifiedNamespace(srvcBusUrl)
						.connectionString(connectionString).sender().queueName(srvcBusQueue).buildClient();
				JSONObject jobj = new JSONObject();
				jobj.put("ordersummary", finalOrder.getProducts());
				ServiceBusMessage sb = new ServiceBusMessage(jobj.toString());
				sb.setSessionId(this.sessionUser.getSessionId());
				senderClient.sendMessage(sb);
			}
			...
			logger.info("All Order details:" + updatedOrder);
