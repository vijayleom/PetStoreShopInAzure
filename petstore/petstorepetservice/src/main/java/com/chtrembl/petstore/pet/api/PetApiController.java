...
import com.postgres.jdbc.data.pets.data.PetsDetailService;
...
public class PetApiController implements PetApi {
	...
	@Override
	public PetsDetailService getDBBeanToBeAutowired() {
		return petsDetailService;
	}
	...
	public ResponseEntity<List<Pet>> findPetsByStatus(
		...
		if (acceptType != null && contentType != null && acceptType.contains("application/json")
			...
			PetApiController.log.info(String.format(
					"Pet details from the DB: %s",
					this.getDBLoadedPets()));
			try {
				//String petsJSON = new ObjectMapper().writeValueAsString(this.getPreloadedPets());
				String petsJSON = new ObjectMapper().writeValueAsString(this.getDBLoadedPets());
